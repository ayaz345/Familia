...
class TopicModelFastMerge(object):
    ...
    def topic_cluster(self, tpc_simhash, hashbits):
        ...
        candidate_topic_cnt = sum(len(item) for item in dis_sets)
        print(
            f"There are {candidate_topic_cnt} candidate topic pairs with the same Simhash value in {len(dis_sets)} clusters."
        )
        return dis_sets
    ...
    def reduce_topic(self, topk, Jac_thresh, hashbits, output_file):
        ...
        print(
            f"Merge {redundant_topic_cnt} redundant topic pairs into {len(dis_sets)} topics (sets)."
        )
        # 对属于同一类的主题进行合并
